...
class GroupGridView extends CGridView {
    ...
    */
	...
	{
			$data=$this->dataProvider->data[$row];
			echo '<tr class="'.$this->evaluateExpression($this->rowCssClassExpression,array('row'=>$row,'data'=>$data)).' extrarow" >';
	}
	else if(is_array($this->rowCssClass) && ($n=count($this->rowCssClass))>0)
		echo '<tr class="'.$this->rowCssClass[$row%$n].' extrarow">';
	else
		echo '<tr>';
        /*echo '<tr>';*/
        ...
        echo '</tr>';
        echo '<tr class="line">';
        echo '<td class="line'.($row==(count($this->dataProvider->data)-1) ? " last" : "").'" colspan="'.$colspan.'">&nbsp;</td>';
        echo '</tr>';
    }
