...
class EMenu extends CWidget
{
	...
	public function init()
	{
		...
		foreach ($this->items as $item) {        
                    if ( (isset($item['visible'])&&$item['visible']) || !isset($item['visible'])) $content .= $this->renderTag($item);
		}
	...
	private function renderTag($item, $level=1)
	{
		...
		if( isset($item['sub']) && is_array($item['sub']) )
		{
			...
			{       
                            if( !empty($sub['active']) ) {                                
                                $act = explode('/', $sub['active']);
                                if ($this->_route[0] == $act[0]) $item['active']= $act[0];
                            }  
                            if ( (isset($sub['visible'])&&$sub['visible']) || !isset($sub['visible'])) {
		...
		if( $level == 1 ){
			...
			$content  = CHtml::tag('div', $toggleOptions, $toggleContent);
				...
				'class' => implode(' ', $toggleclass)
                            ...
                            $toggleContent = $icon . $toggle . $content;
                            $content  = CHtml::tag('span', $toggleOptions, $toggleContent);
                        }    
                }		
