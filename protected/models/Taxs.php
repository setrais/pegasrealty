 ...
 *
 * The followings are the available columns in table 'taxs':
 * @property integer $id
 ...
 */
class Taxs extends CActiveRecord
{
        public $maxname;
        public $minname;
	 ...
	 * @return Taxs the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			'realestates' => array(self::HAS_MANY, 'Realestates', 'tax_id'),
		);
	}
        ...
        {
            return array(        
                'sitemap' => array( 'select' => 'id, title, grid, abbr', 
            ...
            if ($vid) {
                $this->getDbCriteria()->mergeWith( 
                    array( 'condition' => 'EXISTS (SELECT * 
                                             FROM realestates r 
                                             WHERE r.tax_id = t.id AND ((r.realestate_vid_id=:vid) AND (r.ACT IS NULL OR r.ACT=1) 
                ...
                $this->getDbCriteria()->mergeWith( 
                    array( 'condition' => 'EXISTS (SELECT * 
                                             FROM realestates r 
                                             WHERE r.tax_id = t.id AND ((r.ACT IS NULL OR r.ACT=1) 
                                                                   AND (r.DEL IS NULL OR r.DEL=0))
                                             )', 
                         )
                );                
	...
	public function search()
	{
        ...
        public function getLinkMapTitle() {           
            return 'Аренда коммерческой недвижимости Москвы с налогооблажением '.$this->abbr;
        }
        public function getLinkMapText() {
            //return 'Аренда коммерческой недвижимости Москвы с налогооблажением '.$this->abbr;
            return 'Коммерческой недвижимость с налогооблажением '.$this->abbr;
        }
