...
/**
 * This is the model class for table "fstavka_offers".
 *
 * The followings are the available columns in table 'fstavka_offers':
 * @property integer $id
 * @property string $uid
 * @property string $sid
 ...
 * @property Valutes $valute
 */
class FstavkaOffers extends CActiveRecord
{
        public $title;
        public $maxname;
        public $minname;
	 ...
	 * @return FstavkaOffers the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, uid, sid, valute_id, title, grid, init_value, final_value, sort, act, del, desc, anons, detile, description, seo_title, seo_desc, seo_keywords, create_date, update_date', 'safe', 'on'=>'search'),
		);
	}
            ...
            return array(        
                'sitemap' => array( 'select' => 't.id, t.grid, t.final_value, t.init_value', 
                                 'condition' => ' EXISTS (SELECT * FROM realestates r 
                                                         WHERE (r.price BETWEEN t.init_value AND t.final_value) AND ((r.act=1)OR(r.act is NULL))AND((r.del=0)OR(r.del is NULL)))',//'((t.act=1)OR(t.act is NULL))AND((t.del=0)OR(t.del is NULL))'*/
        ...
        /* Exists vid for real in stavka offers 
         * @merge criteria
         */
        ...
        /* Exists vid for real in stavak offers 
         * @merge criteria
         */
            ...
            if ($vid) {
                $this->getDbCriteria()->mergeWith(array(
                    'condition'=>'EXISTS (SELECT * 
                                          FROM realestates r 
                                          WHERE (r.realestate_vid_id=:vid) AND (r.price BETWEEN t.init_value AND t.final_value) 
               ...
               $this->getDbCriteria()->mergeWith(array(
                    'condition'=>'EXISTS (SELECT * 
                                          FROM realestates r 
                                          WHERE (r.price BETWEEN t.init_value AND t.final_value)  
                                                       AND ((r.act=1)OR(r.act is NULL))AND((r.del=0)OR(r.del is NULL))
                                          )',  
               ));    
            }
        ...
        public function realexin($in=null) {
            ...
            $this->getDbCriteria()->mergeWith( 
                array(  'condition'=>'EXISTS (SELECT * 
                                          FROM realestates r 
                                          WHERE (r.id IN ('.$in.')) AND (r.price BETWEEN t.init_value AND t.final_value) 
                                                                           AND ((r.act=1)OR(r.act is NULL))AND((r.del=0)OR(r.del is NULL))
                                          )',  
                     )
            );
        ...
        /* Stavka offers real id
         * @merge criteria
         */
        public function realexsel($inexsel=null,$exsel=null,$exjoin=null) {
            
            if (empty($exsel)) $exsel ='r.price BETWEEN t.init_value AND  t.final_value';                        
