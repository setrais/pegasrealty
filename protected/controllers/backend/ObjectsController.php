...
class ObjectsController extends BackendController
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','gen'),
	...
	{
            $reals = Realestates::model()->findAll(array('group'=>'title','order'=>'id ASC'));
            foreach ($reals as $real) {
                $exist = Objects::model()->exists(array('condition'=>"title='".$real->title."'"));
                if (!$exist) {
                   $model=new Objects;
                   $model->sid = null;
                   $model->uid = null;
                   $model->abbr = $real->title;
                   $model->title = $real->title;
                   $model->create_date=date('Y-m-d H:s:i');
                   $model->update_date=date('Y-m-d H:s:i');                       
                   $this->genIds($model,true);
                   $model->save();
                } 
            }   
            $realestates = Realestates::model()->findAll();
            foreach ($realestates as $realestate) {
                $object = Objects::model()->find(array('condition'=>"title='".$realestate->title."'")); 
                $real = Realestates::model()->findByPk($realestate->id);                
                $real->oid = $object->id;
                $real->update();
            }
	}
