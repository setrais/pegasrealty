...
class IblocksController extends Controller
{
	...
	public function accessRules()
	{
		/*return array(
			...
			array('deny',  // deny all users
                ...
                return array(
                    	array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('getAjaxSection','genTitle', 'genDescription','genKeywords'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
	...
	{
		$dataProvider=new CActiveDataProvider('Iblocks',array(
        ...
        public function actionGetAjaxSection() {            
            if( Yii::app()->request->isAjaxRequest ) {      
                ...
                {                                                   
                    ...
                    if (empty($model->action)) $model->action = $model->typesIblock->action;
                    if (empty($model->url)) $model->url = $model->typesIblock->url_elem.'#id';
                    if (empty($model->url_detile)) $model->url_detile = $model->typesIblock->url_elem.'#id';
                    if (empty($model->url_list)) $model->url_list = $model->typesIblock->url_sect;
