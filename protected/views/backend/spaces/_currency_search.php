	<?php /*<div class="row">
            ...
            <?php if ( $model->valute_id && isset($is_curs) && $is_curs ) : ?> 
                ...
                <div class="currency value">
                   ...
                   <?php foreach ( $curs as $skey=>$val) { ?>                         
                         ...
                         <b>от:</b>
                         <i id="cost_from_<?php echo $val["name"];?>" class="info-field" ><?php echo round($price_from/Yii::app()->currency->$val["name"],5)<>0 
                                        ? round($price_from/Yii::app()->currency->$val["name"],5) : "" ;?></i>&nbsp;
                         <b>до:</b>
                         <i id="cost_to_<?php echo $val["name"];?>" class="info-field" ><?php echo round($price_to/Yii::app()->currency->$val["name"],5)<>0 
        ...
        <script>
            ...
            function loadBlockPromCurrency(price_from,price_to,currency_title,currency_id,is_conv) {                                    
                   var url = '/bax.php/spaces/ajaxshowpromcurrenty?'
                                +'vid='+currency_id+'&'
		   ...
		   $("#block-currency").append(loading);
                   ...
                   $('#block-currency').load(url, function(response, status, xhr) {
                       if ( status =="error") {
                            ...
                            if (currency_id.length>0) {
                                $('#Spaces_valute_id').attr('title',currency_title);
                            }else{
                                $('#Spaces_valute_id').removeAttr('title');
                            }
                            if (currency_id.length>0) {
            ...
            $('#Spaces_valute_id').live('change',
                 
                 function() 
                 {                      
                   var curr_currency = $('#Spaces_valute_id').attr('title');
                   var currency_id = $('#Spaces_valute_id').val();
                   var currency_title = $('#Spaces_valute_id option[value="'+currency_id+'"]').text();                   
