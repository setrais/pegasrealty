...
function() {
    ...
    $.get(url, function(data) {
        $('#list-realestates.popup').dialog({title:'Просмотр недвижимости',
                            modal: true,
                            width: 730,
                            close: function(event, ui) {                                                   
                                /* Clear div editbox for orders grid */
                                $('#list-realestates').empty(); 
                                $('#list-realestates').dialog('destroy'); 
                            }
        });                         
        $('#list-realestates').html(data);                               
    });    
    return false;
}
EOD;
...
function() {
    var url = $(this).attr('href');
    $.get(url, function(data) {
        $('#list-realestates.popup').dialog({title:'Изменение статуса недвижимости',
                            modal: true,
                            width: '280',
                            close: function(event, ui) {       
                                // Clear div editbox for orders grid 
    ...
    //alert('Пока находится в стадии разработки');
    return false;
}
EOD;
?>
...
<?php $this->widget('application.components.widgets.RGridView', array(
	...
	'filter'=>$model,
        ...
        'groupActions'=>array(
                       ...
                       'groupDeleteOrder'=>'Удалить площади', 
                    ),           
	'columns'=>array(
            /*array(
		'class'=>'CButtonColumn',                
           ...
           array( 'name'=>'act',                  
                  'header'=>Yii::t('label','Status'),
                  'type'=>'text',
                  'htmlOptions'=>array('style'=>'text-align:center;width:25'/*60px;'/*25px*/),
                  'value'=>'$data->getStatusForClient('.$model_client->id.');',                   
                  'filter'=>CHtml::listData(RealestateStatus::model()->findAll("(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
         ...
         array(
                'class'=>'CButtonColumn',
                'template'=>'{view} {statused}<br/>{send} {delete}',
                'buttons'=>array( 'view'=>
                                  array( 'label'=>'View',
                                         'url'=>'Yii::app()->controller->createUrl("/realestates/ajaxview",array("id"=>$data->primaryKey,"cid"=>'.$model_client->id.'))',                                             
                                         'click'=>$js_preview,
                                         'visible'=>'true',    
                                  ),
                                  'send'=>
                                  array( 'label'=>'Send',
                                         'url'=>'Yii::app()->controller->createUrl("/realestates/ajaxsendinclient",array("id"=>$data->primaryKey,"cid"=>'.$model_client->id.'))',
                                  ...
                                  'statused'=>
                                  array( 'label'=>'Status',
                                         'url'=>'Yii::app()->controller->createUrl("/realestates/ajaxstatusedinclient",array("id"=>$data->primaryKey,"cid"=>'.$model_client->id.'))',
                                         'imageUrl'=>Yii::app()->request->baseUrl.'/images/icons/recent_changes.png',    
                                         'click'=>$js_statused,
                                         'visible'=>'true',    
                                  ),                       
                           ),                
...
<div id="list-realestates" class="popup"></div>
