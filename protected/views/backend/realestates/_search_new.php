...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
    	...
    	<div class="row">
            ...
            <?php $data = CHtml::listData(RealestateTypes::model()->findAll(), 'id', 'title'); ?>
	...
	<div class="row">
            ...
            <?php $data = CHtml::listData(Areas::model()->findAll("(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                         array("order"=>"sort")), 'id', 'title'); ?>
            <?php $this->widget('ext.widgets.EchMultiSelect', array(
                          // the data model associated with this widget:
                          'model' => $model,
	...
	<div class="row" >
		<?php echo $form->labelEx($model,'number_tax'); ?>
                <?php echo $form->dropDownList($model,'number_tax',   
                              CHtml::listData(TaxReference::model()->findAll("(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                                array("order"=>"sort")), 'id', 'abbr'), array('prompt'=>'--'.Yii::t('all','Select')."--")); ?>                                     
		<?php// echo $form->textField($model,'realestate_class_id'); ?>
	...
	<div class="row">
        ...
        <div class="row">
            ...
            <?php $data =  CHtml::listData(Realestates::model()->findAll($isId ? 'not id='.$model->id : ''), 'id', 'title'); ?>
            <?php // Другая площадь ?>
            <?php $this->widget('ext.widgets.EchMultiSelect', array(
                          // the data model associated with this widget:
                          'model' => $model,
