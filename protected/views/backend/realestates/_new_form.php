...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<?php echo $form->errorSummary($model); ?>
	  ...
	  <?php// echo $form->textField($model,'areas_id'); ?>
          <?php/* echo $form->dropDownList($model,'areas_id',
                        CHtml::listData(Areas::model()->findAll(array("condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                          "order"=>"title")), 'id', 'title'), array('prompt'=>'--'.Yii::t('all','Select')."--")); */?>
          <?php $data=CHtml::listData(Areas::model()->findAll(array("condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
	  ...
	  <?php echo $form->labelEx($model,'remoteness',array('class'=>'many')); ?>                                
	  <?php echo $form->textField($model,'remoteness',array('size'=>2,'maxlength'=>3,
	  ...
	  <?php echo $form->error($model,'remoteness'); ?>                
          <?php echo $form->dropDownList($model,'unit_id',
                         ...
                         array("order"=>"sort")), 'id', 'short_title'), array('prompt'=>Yii::t('all','--'))); ?>    		
	  <?php echo $form->error($model,'unit_id'); ?>                
	</div>
        <?php $bcnt=0; ?>                           
        <?php if (!$model->metros) :  $metro = new RealestateMetros(); ?>
        <div class="row metros" id="metros">
                <?php echo $form->labelEx($metro,'metro_id'); ?>
                <?php $data = CHtml::listData(Metros::model()->findAll(), 'id', 'title'); ?>
                <?php $this->widget('ext.widgets.EchMultiSelect', array(    
                                // the data model associated with this widget:
                                'model' => $metro,
                                ...
                                'data' => $data,    
                                // the name of the drop down list (this must be set if 'model' and 'dropDownAttribute' are not set):
                                'name' => 'RealestateMetros_metro_id',    
                                // additional HTML attributes for the drop down list:                           
                                'dropDownHtmlOptions'=> array(
                                  'style'=>'width:auto'/*'width:378px;'*/, 
                                  'options'=>array($metro->metro_id),  
                                  'multiple'=>false,  
                                  'prompt'=>'--'.Yii::t('all','Select')."--",  
                                  'id'=>'RealestateMetros_metro_id'.($bcnt>0 ? $bcnt : ''),  
                                  'name'=>'RealestateMetros[metro_id][]',    
                                  'ajaxRefresh'=>true,  
                                ),
                                // options for the jQuery UI MultiSelect Widget
                                // (see the project page for available options):
                ...
                <?php echo $form->error($metro,'metro_id'); ?>            
                <?php echo $form->labelEx($metro,'remoteness',array('class'=>'many')); ?>                                
                <?php echo $form->textField($metro,'remoteness',array('size'=>2,'maxlength'=>3,'name'=>'RealestateMetros[remoteness][]',
                                                                      'value'=>($metro->remoteness===null ? $metro->remoteness : round($metro->remoteness )))); ?>
                <?php echo $form->error($metro,'RealestateMetros[remoteness]'); ?>                     
                <?php echo $form->dropDownList($metro,'unit_id',
                               CHtml::listData(Units::model()->findAll("(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                               array("order"=>"sort")), 'id', 'short_title'), array('prompt'=>Yii::t('all','--'),'name'=>'RealestateMetros[unit_id][]')); ?>    		

                <?php echo $form->error($metro,'unit_id'); ?>                  
                <?php echo CHtml::link( CHtml::image('/images/add_item.png',Yii::t('form','Add')), '#',
                                           array('onclick'=>"                                                                                                              
                                                             var data = $(this).serialize();                                                                                       
                                                             var cnt = $('#cnt_metros').val();
                                                             var metro = $('#metros');
                                                             var metroLast = $('.row.metros:last');       

                                                             $('#cnt_metros').val(parseInt(cnt)+1);
                                                             var new_cnt=$('#cnt_metros').val();
                                                             
                                                             metroLast.after( $('<div id=\"metros-'+new_cnt+'\" class=\"row metros\">'+metro.html().replace('selected=\"selected\"','')+'<a id=\"DeleteMetro'+new_cnt+'\" href=\"#\" onclick=\"$(\'#metros-'+new_cnt+'\').remove();\"><img alt=\"Удалить\" src=\"/images/del_item.png\"></a></div>'));
                                                             
                                                             $('#metros-'+new_cnt+' #AddMetro').attr('id', $('#metros-'+new_cnt+' #AddMetro').attr('id')+new_cnt);                                                                  
                                                             $('#metros-'+new_cnt+' #RealestateMetros_metro_id').next().remove();
                                                             $('#metros-'+new_cnt+' #RealestateMetros_metro_id').attr('id',  $('#metros-'+new_cnt+' #RealestateMetros_metro_id').attr('id')+new_cnt);                                                                                                                               
                                                             $('#metros-'+new_cnt+' #RealestateMetros_remoteness').attr('id',  $('#metros-'+new_cnt+' #RealestateMetros_remoteness').attr('id')+new_cnt);                                                                                                                               
                                                             $('#metros-'+new_cnt+' #RealestateMetros_unit_id').attr('id',  $('#metros-'+new_cnt+' #RealestateMetros_unit_id').attr('id')+new_cnt);                                                                                                                               
                                                             $('#RealestateMetros_metro_id'+new_cnt).multiselect({'checkAllText':'Выбрать все','uncheckAllText':'Отменить','selectedText':'# выбрано','noneSelectedText':'-- Выбор опции --','multiple':false,'filter':true,'ajaxRefresh':false,'selectedList':5,'header':true,'height':100}).multiselectfilter({'label':'Фильтр:','placeholder':'Ввод слов'});                        
                                                             $('#metros-'+new_cnt+' #RealestateMetros_remoteness'+new_cnt).val('');  
                                                             $('#metros-'+new_cnt+' #RealestateMetros_unit_id'+new_cnt).val('');                                                                                                                                                                                            
                                                  ", 
                                                  'id'=>'AddMetro'));?>
	  </div>             
          <?php else : ?>            
          <?php foreach ($model->metros as $metro) { ?>   
            <div class="row metros" id="metros<?php echo ($bcnt ? "-".$bcnt : "");?>">
                <?php echo $form->labelEx($metro,'metro_id'); ?>
                <?php $data = CHtml::listData(Metros::model()->findAll(), 'id', 'title'); ?>
                <?php $this->widget('ext.widgets.EchMultiSelect', array(    
                                // the data model associated with this widget:
                ...
                <?php echo $form->textField($metro,'remoteness',array('size'=>2,'maxlength'=>3,'name'=>'RealestateMetros[remoteness][]',
                                                                      'value'=>($metro->remoteness===null ? $metro->remoteness : round($metro->remoteness )))); ?>
                <?php echo $form->error($metro,'remoteness'); ?>                     
                ...
                <?php echo CHtml::link( CHtml::image('/images/add_item.png',Yii::t('form','Add')), '#',
                                           array('onclick'=>"                                                                                                              
                                                  ...
                                                  ", 
                                                  'id'=>'AddMetro'));?>                
                <?php echo CHtml::link( CHtml::image('/images/del_item.png',Yii::t('form','Delete')), '#', 
                                              array('onclick'=>"$('#metros-".$bcnt."').remove();",
                                                   'id'=>'DeleteMetro'));?>    
                <?php else : ?>
                <?php echo CHtml::link( CHtml::image('/images/add_item.png',Yii::t('form','Add')), '#',
                                           array('onclick'=>"                                                                                                              
          ...
          <?php } ?>            
          <?php endif; ?>  
          <?php echo CHtml::hiddenField('cnt_metros',$bcnt,array('id'=>'cnt_metros')); ?>          	
	...
	<div class="row" >
        ...
        <div class="row">
            <?php echo $form->labelEx($model,'date_rang',array('label'=>'Дата и время звонка')); ?>
            <?php $langs = array_flip(Yii::app()->params->languages); 
                  if ($model->date_rang) 
                      $model->date_rang = date('d.m.Y H:i',  strtotime($model->date_rang));
                  else 
                      $model->date_rang = date('d.m.Y H:i',  time());
            ?>
            <?php $this->widget ('ext.CJuiDateTimePicker.CJuiDateTimePicker',
                array(
                    'model'=>$model, //Model object
                    'attribute'=>'date_rang', //attribute name
			...
			'buttonImageOnly'=> true,
                    ...
                    'htmlOptions'=>array('size'=>14), // jquery plugin options
                ));
           ?>           
            <?php/* $langs = array_flip(Yii::app()->params->languages); 
                  if ($model->date_rang) 
                      ...
                      $model->date_rang = date('d.m.Y',  time());
            ?>
	...
	<div class="row">
        ...
        <div class="row">
		<?php echo $form->labelEx($model,'Предназначения'); ?> 
                <?php $data = CHtml::listData(Destinations::model()->findAll("(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                                array("order"=>"sort")), 'id', 'title'); ?> 
                <?php// Предназначения недвижимости ?>
                <?php// echo "<nobr>".CHtml::checkBoxList('Realestates[realestateDestinations]', $selDestinations, $data, array("separator"=>"&nbsp;"))."</nobr>" ;?>
                <?php $this->widget('ext.widgets.EchMultiSelect', array(
                          // the data model associated with this widget:
                          'model' => $model,
                          // the attribute associated with drop down list of this widget:
                          'dropDownAttribute' => 'realestateDestinations',     
                          // data for generating the options of the drop down list:
                          'data' => $data,                            
                          // the name of the drop down list (this must be set if 'model' and 'dropDownAttribute' are not set):
                          'name' => 'Realestates_realestateDestinations',    
                          // additional HTML attributes for the drop down list: 
                          'dropDownHtmlOptions'=> array(
                            'style'=>'width:auto'/*'width:378px;'*/,
                            'multiple'=>true,
		...
		<?php echo $form->error($model,'realestateDestinations'); ?>
	</div>
	...
	<div class="row" >
		<?php echo $form->labelEx($model,'number_tax'); ?>
                <?php echo $form->dropDownList($model,'number_tax',   
                              CHtml::listData(TaxReference::model()->findAll(array("condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                                     "order"=>"num")), 'id', 'abbr'), array('prompt'=>'--'.Yii::t('all','Select')."--")); ?>                                     
		<?php// echo $form->textField($model,'realestate_class_id'); ?>
	...
	<div class="row">
		...
		<?php echo $form->labelEx($model,'realestateOthers'); ?>                	
                <?php $data =  CHtml::listData(Realestates::model()->findAll(($isId && !isset($copid)) ? 'not id='.$model->id : ''), 'id', 'title'); ?>
                <?php // Другая площадь
                      if ($isId) {    
		...
		<?php echo $form->labelEx($model,'realestateSimilarities'); ?>                		
                <?php $data =  CHtml::listData(Realestates::model()->findAll($isId ? 'not id='.(isset($copid) ? $copid : $model->id) : ''), 'id', 'title'); ?>
				...
				beforeSend: function(){
					var loading = $("<div>", {
                                ...
                                success: function(data) {
                                    ...
                                    $("#Representatives_telephone_1").val(data.telephone_1);
                                    $("#Representatives_telephone_2").val(data.telephone_2);
                                    $("#Representatives_telephone_3").val(data.telephone_3);
                                    $("#Representatives_fax").val(data.fax);
                                    $("#Representatives_email").val(data.email);
