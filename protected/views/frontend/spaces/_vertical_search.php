...
?>
   <div class="p-5" >
       <div class="panel-params fl w-248" >
          <div class="rows odd radius-5-0">               
               <div class="oper">
                   ...
                   <div class="checks">
                     ...
                     <?php echo "<nobr>".CHtml::checkBoxList("Spaces[operation_id]", "1", $data, array("separator"=>"&nbsp;"))."</nobr>" ;?>
                   </div>
               </div>    
          </div>    
          <div class="rows oven">               
          ...
          <div class="rows odd">               
               <div class="oper">
                   ...
                   <div class="checks">
                      ...
                      <?// print_r($data) ?> 
                      <?php echo CHtml::checkBoxList("Spaces[district_id]", ( trim($_POST["Spaces"]["district_id"])<>'' ? $_POST["Spaces"]["district_id"] : ""/*array("7")*/ ), $data, array("separator"=>" ","class"=>"boxlist","template"=>"<nobr class=\"item\" >{input} {label}</nobr>")) ;?>
                   </div>
               </div>    
          </div>                          
          <div class="rows oven">               
               <div class="oper">
                   ...
                   <div class="checks p-l5">
                      ...
                      <?php echo CHtml::checkBoxList( "Spaces[unit_id]", trim($_POST["Spaces"]["unit_id"])<>"" ? $_POST["Spaces"]["unit_id"] : ""/*array("2")*/, $data, array("separator"=>" ","template"=>"<nobr class=\"item\" >{input} {label}</nobr>")) ;?>
                      </div>    
                   </div>
               </div>   
          </div>              
          <div class="rows odd">               
               <div class="oper">
                   <div class="title"><?php echo $form->label($model,'space_vid_id'); ?></div>
                   <div class="checks">
                      <?php $data = CHtml::listData(SpaceVids::model()->findAll( array( "condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)", "order"=>"sort_main" )), 'id', 'title'); ?>
                      <?php echo CHtml::checkBoxList("Spaces[space_vid_id]", trim($_POST["Spaces"]["space_vid_id"])<>"" ? $_POST["Spaces"]["space_vid_id"] : ""/*array("1")*/, $data, array("separator"=>" ","template"=>"<nobr class=\"item w-112\" >{input} {label}</nobr>")) ;?>
                   </div>
               </div>   
          </div>             
          <div class="rows oven">               
               <div class="oper">
                   <div class="title"><?php echo $form->label($model,'space_class_id'); ?></div>
                   <div class="checks">
                      <?php $data = CHtml::listData(SpaceClasses::model()->findAll( array( "condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",
                                        "order"=>"sort")), 'id', 'abbr'); ?>
                      <?php echo CHtml::checkBoxList("Spaces[space_class_id]", trim($_POST["Spaces"]["space_class_id"])<>"" ? $_POST["Spaces"]["space_class_id"] : ""/*array("1")*/, $data, array("separator"=>" ","template"=>"<nobr class=\"item-class\" >{input} {label}</nobr>")) ;?>
                   </div>
               </div>   
          </div>                               
          <div class="rows odd">               
          ...
          <div class="rows oven">               
               <div class="oper">
                   ...
                   <div class="checks">
                      ...
                      <div class="box-checks valign-m w-40">
                      <?php echo CHtml::checkBoxList( "Spaces[valute_id]", trim($_POST["Spaces"]["valute_id"])<>"" ? $_POST["Spaces"]["valute_id"] : array("2"),  $data, array("separator"=>" ","template"=>"<nobr class=\"item w-54\" >{input} {label}</nobr>")) ;?>
                      </div>    
                   </div>
               </div>   
          </div>               
          ...
          <div class="rows oven bann valign-m halign-c">  
       ...
       <div class="panel-map fl <?/*w-660*/?>" >  
            ...
            <div class="contenerMap" style="display:block;">
                <?php $this->renderPartial( '/spaces/_vmap_metros',                                                
                        array('metros'=>$metros)
                ); 
                ?>
            </div>
