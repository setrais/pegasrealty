...
$property = Properties::model()->findByPk($model->realestatesProperties);
 
$this->pageTitle= Yii::t('all','Коммерческая недвижимость')
                    .($property
                    ? ' | Аренда коммерческой недвижимости со свойством '.$property->title.' без комиссии в Москве.'
                     .' Снять нежилое помещение в торговом, бизнес центре, особняке, банке, жилом доме, комлексе, парке либо'
                     .' здание целиком с свойством '.$property->title.','
                     .' без посредников от собственника Москвы.'
                    : '  | Аренда коммерческой недвижимости по свойствам в Москве без комиссии. Снять нежилое (офисное, банковское, торговое)'
                     .' помещение либо бизнес центр, банк, здание, особняк целиком с различными свойствами, без посредников от собственника Москвы.');

if ($property) $this->pageDescription= trim($property->seo_desc)<>'' 
                                        ? 'Здесь отобраны предложения по аренде коммерческой недвижимости со свойством '.mb_strtolower($property->title, 'UTF-8').' в Москве. '
                                          .$property->seo_desc 
                                        : 'Здесь отобраны предложения по аренде коммерческой недвижимости со свойством '.mb_strtolower($property->title, 'UTF-8').' в Москве. '
                                         ...
                                         .'медцентре, банке Москвы, а также аренду бизнес центров, банков, особняков и зданий целиком обладающего свойством '
                                         .mb_strtolower($property->title, 'UTF-8')
                                         .'. Снимаемая в аренду коммерческая недвижимость имеющая свойство '.mb_strtolower($property->title,'UTF-8').' также отличается видом,' 
                                         .' объектом размещения, адресом, классом, назначением, удаленностью от метро, площадью, стоимостью, валютой.';                   
else $this->pageDescription=Yii::t('all','Здесь отобраны предложения аренды коммерческой недвижимости по свойствам в Москве. Предложения включают аренду ' 
                                        ...
                                        .'аренду бизнес центров, банков, особняков и зданий целиком отличающихся между собой свойствами. Снимаемая в аренду коммерческая недвижимость '
                                        .'кроме этого различна по виду, объектом размещения, адресу, назначению, удаленности от метро, площади, стоимостью, и валюты расчетов.');

if ($property->title) $this->pageKeywords=trim($property->seo_desc)<>'' 
                                          ? array_merge(explode(',', 'аренда со свойством '.mb_strtolower($property->title,'UTF-8')
                                                  ...
                                                  .', аренда в москве '.mb_strtolower($property->title,'UTF-8')                                                  
                                                  //.', аренда офиса москвы '.mb_strtolower($property->title,'UTF-8')
                                                  .', '.mb_strtolower($property->title,'UTF-8')
                                                  .', каталог недвижимости, коммерческая недвижимость, недвижимость, аренда, москва, москвы, в москве, свойство, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц'
                                                  ),  
                                                 ...
                                                 $short ? HRu::iskeyintxt(explode(',',$property->seo_keywords),                                                   
                                                        ...
                                                        : explode(',',trim($property->seo_keywords))
                                          )
                                          : explode(',', 'аренда со свойством '.mb_strtolower($property->title,'UTF-8')
                                                  .', аренда '.mb_strtolower($property->title,'UTF-8')
                                                  .', аренда '.mb_strtolower($property->title,'UTF-8').' москва'
                                                  .', аренда '.mb_strtolower($property->title,'UTF-8').' в москве'
                                                  ...
                                                  .', '.mb_strtolower($property->title,'UTF-8')
                                                  .', каталог недвижимости, коммерческая недвижимость, недвижимость, аренда, москва, москвы, в москве, свойство, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');                   
else $this->pageKeywords=explode(',', 'аренда в москве, каталог недвижимости, коммерческая недвижимость, недвижимоcть, аренда, москва, в москве, москвы, использование, назначение, предназначение, свойство');    
...
if ($property) {
    $this->breadcrumbs=array(	
	 ...
	 "Аренда коммерческой недвижимости cо свойством ".mb_strtolower($property->title,'UTF-8')." в Москве",
    );
} else {
    $this->breadcrumbs=array(	
         Yii::t('menu-adm','List Realestates')=>array('index'),
	 'Аренда коммерческой недвижимости по свойствам',
    );
}
...
<?php echo //Yii::t('all','Коммерческая недвижимость').' | '.
           ($property 
                    ? 'Аренда коммерческой недвижимости cо свойством '.mb_strtolower($property->title, 'UTF-8').' без комиссии в Москве.'
                     .' Снять нежилое помещение или здание целиком имеющее свойство '.mb_strtolower($property->title,'UTF-8')                       
                     .' без посредников по стоимости от собственника Москвы.'
                     //.' без посредников по стоимости аренды от собственника Москвы.'                     
                    : 'Аренда коммерческой недвижимости по свойствам в Москве, без комиссии. Снять нежилое помещение либо здание Москвы целиком 
                       с различными свойствами, без посредников по стоимости аренды от собственника.');
?>      
</h1>
...
<?php
    // Переменная $extraRowExpression    
    include __DIR__ . '/include/_var_extrowexp_property.php';
?>
...
<?php 
    $model->search()->pagination->route = Yii::app()->createUrl('/realestates/property', array('id'=>$property->id));
?>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	...
	'dataProvider'=>$model->search($short ? 8 : 10), 
        ...
        //'ajaxUrl'=>Yii::app()->createUrl('/realestates/property'),
        'ajaxUrl'=>$propertie->is_all ? Yii::app()->createUrl('/realestates/property') : Yii::app()->createUrl('/realestates/property',array('id'=>$property->id)),
        'beforeAjaxUpdate'=> $beforeAjaxUpdate,
        'afterAjaxUpdate'=> $afterAjaxUpdate,
	'filter'=>$model,
        'summaryText'=>Yii::t('core',                 
                              'Displaying Items {start} - {end} of {count} results', 
                               $model->search($short ? 5 : 10)->getTotalItemCount()),
	...
	'columns'=>array(
		...
		'class'=>'CButtonColumn',                
           ...
           array( 'name'=>'realestatesProperties',
                  'header'=> Yii::t('all','Cвойство'),  
                  'type'=> 'html',
                  'filter'=>!$propertie->is_all ? CHtml::activeHiddenField($model,'realestatesProperties') : null,
                  'value'=>'$data->getProperty(false,"'.$property->title.'")',  
                  'htmlOptions'=>array('style'=>'text-align:center;width:50px;font-size:0.8%'),
                  'headerHtmlOptions'=>array('width'=>'50'),
                  'visible'=>!$propertie->is_all,
           ),                 
           array( 'name'=>'realestatesProperties',
                  'header'=> Yii::t('all','Cвойства'),  
                  'type'=>'html',
                  'value'=>'$data->getProperty(false,false,", ",60)',
                  'htmlOptions'=>array('style'=>'text-align:center;width:50px;font-size:0.8%'),
                  'filter'=>CHtml::listData(Properties::model()->findAll(
                                                array("condition"=>"(t.act IS NULL OR t.act=1) AND (t.del IS NULL OR t.del=0)",
                                                      "select"=>'t.id,t.title',
                                                      "order"=>"t.sort")
                                            ), 'id', 'title'),
