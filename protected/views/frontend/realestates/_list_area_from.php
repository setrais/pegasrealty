...
if (Yii::app()->getRequest()->getParam('area-from')) {
    $offer = FareaOffers::model()->find('init_value='.Yii::app()->getRequest()->getParam('area-from'));
} 
if (Yii::app()->getRequest()->getParam('id')) {
    $offer = FareaOffers::model()->findByPk(Yii::app()->getRequest()->getParam('id'));
}   

$this->pageTitle=Yii::t('all','Коммерческая недвижимость') // Возможно использовать $offer->seo_title и $property->seo_title ну это уже другая история
                .($offer 
                  ? ($property->is_ceil 
                     ? ' | Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2 '.' в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' площадью от '.$offer->init_value.' м2 целиком, без посредников по стоимости аренды'
                      .' от собственника в административном округе Москвы.'
                     : ' | Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2 '.' в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' площадью от '.$offer->init_value.' м2 без посредников по стоимости аренды'
                      .' от собственника в административном округе Москвы.')        
                  : ($property->is_ceil 
                     ? ' | Аренда '.mb_strtolower($property->namewhats,'UTF-8').' по площади в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' различной площади целиком, без посредников по стоимости аренды от собственника'
                      .' в административном округе Москвы.' 
                     : ' | Аренда '.mb_strtolower($property->namewhats,'UTF-8').' по площади в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' различной площади, без посредников по стоимости аренды от собственника'
                      .' в административном округе Москвы.'));


// Если есть seo описание offer или property
if ($offer) $this->pageDescription= trim($offer->seo_desc)<>'' 
                                        ? 'Здесь отобраны предложения по аренде '.mb_strtolower($property->namewhats,'UTF-8')
                                          .' площадью от '.$offer->init_value.' м2'.' в административных округах Москвы. '                                          
                                          .$offer->seo_desc 
                                        : 'Здесь отобраны предложения по аренде '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '
                                          .$offer->init_value.' м2'.' в административных округах Москвы.';                   
else $this->pageDescription= 'Здесь отобраны предложения по аренде '.mb_strtolower($property->namewhats,'UTF-8').' различной площади в административных округах Москвы. ';
    ...
    if ($offer) {
        $this->pageDescription = ($property->is_ceil 
                                  ? 'Предложения включают аренду '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '
                                    .$offer->init_value.' м2'.' целиком, в административных округах Москвы. '
                                   .'Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2'
                                  ...
                                  : 'Предложения включают аренду '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '
                                   ...
                                   .' Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '
                                    .$offer->init_value.' м2'.' в Москве отличается объектом размещения, адресом, назначением, свойствами,'
                                    .' удаленностью от метро, классом, стоимостью, валютой.');
    }else{
        $this->pageDescription = ($property->is_ceil 
                                  ? 'Предложения включают аренду '.mb_strtolower($property->namewhats,'UTF-8').' по площади целиком, в административных округах'
                                   .' Москвы. Аренда '.mb_strtolower($property->namewhats,'UTF-8').' по площади в Москве'
                                  ...
                                  : 'Предложения включают аренду '.mb_strtolower($property->namewhats,'UTF-8').' по площади '
                                   .' в административных округах Москвы.'
                                   .' Аренда '.mb_strtolower($property->namewhats,'UTF-8').' различной площади в Москве отличается объектом размещения, адресом, назначением,'
...
if ($offer) $this->pageKeywords=trim($offer->seo_keywords)<>'' 
                                ? array_merge( explode(',', //'аренда '.mb_strtolower($property->namewhats,'UTF-8').' от '.$offer->init_value.' м2'                                                                                                                  
                                                         ...
                                                         .', аренда'.' '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2'
                                                         .', аренда'.' '.mb_strtolower($property->namewhats,'UTF-8').' в москве площадью от '.$offer->init_value.' м2'
                                                         .', аренда'.' '.mb_strtolower($property->namewhats,'UTF-8').' москвы площадью от '.$offer->init_value.' м2'                                                         
                                                         ...
                                                         .($property->title 
                                                          ...
                                                          .($property->abbr 
                                                            ...
                                                            .', площадь, от, площадью, больше, м2, кв.м, квадратных метров, каталог, недвижимость, аренда, москва, в москве, москвы, класса, коммерческой недвижимости, округ, район, метро, удаленность, вид, свойства, площадь, м2, планировка, парковка, стоимость'),
                                ...
                                : explode(',', //'аренда '.mb_strtolower($property->namewhats,'UTF-8').' от '.$offer->init_value.' м2'                                                                                                                  
                                                         ...
                                                         .($property->title 
                                                          ...
                                                          .($property->abbr 
                                                                        ...
                                                                        .', '.mb_strtolower($property->abbr,'UTF-8') : '')  
                                                            .', площадь, от, площадью, больше, м2, кв.м, квадратных метров, каталог, недвижимость, аренда, москва, в москве, москвы, класса, коммерческой недвижимости, округ, район, метро, удаленность, вид, свойства, площадь, м2, планировка, парковка, стоимость');                   
else $this->pageKeywords=explode(',', 'аренда в москве, аренда москвы, аренда с площадью, площадь, площадью, м2, кв.м, квадратных метров, каталог недвижимости, недвижимость, аренда, москва, в москве, москвы, класса, коммерческой недвижимости, округ, район, улица, метро, удаленность, вид, свойства, площадь, планировка, парковка, стоимость в месяц, налогооблажение');    
...
if ($offer&&$property->namewhats) {
        ...
        $this->breadcrumbs=array(	
             ...
             'по площади в округах'=>array('realestates/areaFrom'),
             'площадь от '.$offer->init_value.' м2'=>Yii::app()->createUrl('realestates/areaFrom',array('id'=>$offer->id)),
             'по видам'=>array('realestates/vid'),
              mb_strtolower($property->nameov,'UTF-8')=>Yii::app()->createUrl('realestates/vid',array('id'=>$property->id)),
             'Аренда '.mb_strtolower($property->namewhats,'UTF-8')." площадью от ".$offer->init_value.' в адм.округах Москвы' 
        );
        ...
        $this->breadcrumbs=array(	
             ...
             'по видам'=>array('realestates/vid'),
              mb_strtolower($property->nameov,'UTF-8')=>Yii::app()->createUrl('realestates/vid',array('id'=>$property->id)),
             'Аренда '.mb_strtolower($property->namewhats,'UTF-8')." площадью от ".$offer->init_value.' м2'.' в адм.округах Москвы'
        );
} else if ($offer && !$property->namewhats) {
        ...
        $this->breadcrumbs=array(	
             ...
             'площадь от '.$offer->init_value.' м2'=>Yii::app()->createUrl('realestates/areaFrom',array('id'=>$offer->id)),
             'по видам'=>array('realestates/vid'),
             'Аренда коммерческой недвижимости площадью от '.$offer->init_value.' м2'.' в адм.округах Москвы' 
        );    
} else {
        $this->breadcrumbs=array(	
             Yii::t('menu-adm','List Realestates')=>array('index'),
             'по площади в округах'=>array('realestates/areaFrom'),
             'по видам'=>array('realestates/vid'),
             'Аренда коммерческой недвижимости различной площади в адм.округах Москвы'
        );
}
                ...
                ($offer
                  ? ($property->is_ceil 
                     ? 'Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2'.' в Москве без комиссии.'
                      ...
                      .' от собственника в административном округе Москвы.'
                     : 'Аренда '.mb_strtolower($property->namewhats,'UTF-8').' площадью от '.$offer->init_value.' м2'.' в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' площадью от '.$offer->init_value.' м2'
                      .' без посредников от собственника в административном округе Москвы.')
                  : ($property->is_ceil 
                     ? 'Аренда '.mb_strtolower($property->namewhats,'UTF-8').' по площади в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' различной площади целиком, без посредников по стоимости аренды'
                      .' от собственника в административном округе Москвы.' 
                     : 'Аренда '.mb_strtolower($property->namewhats,'UTF-8').' по площади в Москве без комиссии.'
                      .' Снять '.mb_strtolower($property->title,'UTF-8').' различной площади, без посредников по стоимости аренды от собственника'
                      .' в административном округе Москвы.'));
...
<? } ?>        
<?php 
    //$model->search()->pagination->route = Yii::app()->createUrl('/realestates/areaFrom', array('id'=>$offer->id)); 
    $model->search()->pagination->route = Yii::app()->createUrl('/ru/realestates/areaFrom/'.$offer->id.'/vid/'.$property->id);
...
<?php $this->widget('application.components.widgets.DSizerListView', array(
	...
	'itemView'=>'_view_column',
        ...
        'afterAjaxUpdate'=> $afterAjaxUpdate,
         //'ajaxUrl'=>Yii::app()->createUrl('/ru/realestates/areaFrom/'.$offer->id.'/vid/'.$property->id),
)); ?>    
