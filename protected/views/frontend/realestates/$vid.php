<?php
$short = true;
if ($model->realestateVid->title) $this->pageTitle=Yii::t('all','Каталог коммерческой недвижимости ')
                    .': аренда '.mb_strtolower($model->realestateVid->namewhats,'UTF-8').' в Москве';//.' - '.str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));
else $this->pageTitle=Yii::t('all','Каталог коммерческой недвижимости: аренда коммерческой недвижимости, различных видов в Москве ');//." | ".str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));

if ($model->realestateVid->title) $this->pageDescription=trim($model->realestateVid->seo_desc)<>''
                                                         ? Yii::t('all','Здесь отобран каталог коммерческой недвижимости по аренде '.mb_strtolower($model->realestateVid->namewhats,'UTF-8').' в Москве')
                                                           .'. '.trim($model->realestateVid->seo_desc)
                                                         : Yii::t('all','Здесь отобран каталог коммерческой недвижимости по аренде '.mb_strtolower($model->realestateVid->namewhats,'UTF-8').' в Москве.');                   
else $this->pageDescription=Yii::t('all','Здесь отобран каталог всех видов коммерческой недвижимости');
...
$this->breadcrumbs=array(	
	 ...
	 $model->realestateVid->title ? "Аренда ".mb_strtolower($model->realestateVid->namewhats,'UTF-8')." в Москве" 
                                      : 'Аренда всех видов коммерческой недвижимости ',
);
...
<h1>
<?php if ($model->realestateVid->title) echo Yii::t('menu-adm','List Realestates')
                    .': '.'аренда '.mb_strtolower($model->realestateVid->namewhats,'UTF-8').' в Москве';
   else echo Yii::t('menu-adm','List Realestates').': '.'аренда всех видов коммерческой недвижимости в Москве';
?>
</h1>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	'id'=>'realestates-grid',          
	'dataProvider'=>$model->search($short ? 12 : 10),
        ...
        'ajaxUrl'=>Yii::app()->createUrl('/realestates/vid'),
...
<?php } else { ?>
    <br/>
<?php } ?>    
<?php if ( $model->realestateVid->detile ) {  ?>    
<div class="desc m-b18 fs-12 ff-tahoma">    
    <?php if ($short) { ?>
...
<?php } ?>    
<?php if ( $model->realestateVid->description ) {  ?>    
<div class="desc m-b18 fs-12 ff-tahoma">    
    <?php if ($short) { ?>
...
<? } ?>        
<?php  if (!$short) {
            $this->renderPartial('/site/_list_areas', array('model'=>$model));  
       }  
?>              
</div>
<script>
