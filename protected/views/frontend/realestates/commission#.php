<?php
if ($model->commission->title) $this->pageTitle=Yii::t('all','Каталог коммерческой недвижимости ')
                    .': аренда коммерческой недвижимости в Москве, комиссия '.$model->commission->abbr;//." - ".str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));
else $this->pageTitle=Yii::t('all','Каталог коммерческой недвижимости: аренда коммерческой недвижимости в Москве, все комиссии');//." - ".str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));

if ($model->commission->title) $this->pageDescription=Yii::t('all','Здесь отобраны предложения по аренде коммерческой недвижимости Москвы, комиссии ').$model->commission->abbr;                   
else $this->pageDescription=Yii::t('all','Здесь отобраны предложения по аренде коммерческой недвижимости Москвы, с различной коммисией');

if ($model->commission->title) $this->pageKeywords=explode(',', 'аренда комиссия '.mb_strtolower($model->commission->abbr,'UTF-8')
                                                               .', аренда '.mb_strtolower($model->commission->abbr,'UTF-8')
                                                               .', аренда в москве '.mb_strtolower($model->commission->abbr,'UTF-8')
                                                               .', '.mb_strtolower($model->commission->abbr,'UTF-8')
                                                               .', аренда комиссии, каталог недвижимости, коммерческая недвижимость, недвижимость, аренда, офис, комиссия, в москве, москва, москвы, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');                   
else $this->pageKeywords=explode(',', 'аренда в москве, каталог недвижимости, коммерческая недвижимоcть, недвижимость, аренда, офис, коммисия, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');    


$this->breadcrumbs=array(	
         Yii::t('menu-adm','List Realestates')=>array('index'),
	 $model->commission->title ? $model->commission->title : ' все виды комиссии ',	 
);
...
<h1>
<?php if ($model->commission->title) echo Yii::t('menu-adm','List Realestates')
                    .': аренда коммерческой недвижимости с '.'комиссией '.$model->commission->abbr;
   else echo Yii::t('menu-adm','List Realestates').': аренда коммерческой недвижимости с различной коммисией';
?>
</h1>
<?php if ($model->commission->desc ) {  ?>    
<div class="desc m-b18 fs-11 ff-Arial justify">    
    <?php echo $model->commission->desc; ?>
</div>
<? } ?>   
...
<?php 
    $model->search()->pagination->route = Yii::app()->createUrl('/realestates/commission', array('id'=>$model->commission_id));
?>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	...
	'dataProvider'=>$model->search(),
        ...
        'ajaxUrl'=>Yii::app()->createUrl('/realestates/commission'),
