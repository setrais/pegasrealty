...
$this->pageTitle= Yii::t('all','Коммерческая недвижимость')
                    .($model->parking 
                    ? ' | Аренда коммерческой недвижимости в Москве без комиссии, только '.mb_strtolower($model->parking->title,'UTF-8').'.'
                     .' Снять нежилое помещениев торговом, бизнес центре, особняке, банке, здании, жилом доме, комлексе, парке'
                     .' либо здание c '.mb_strtolower(str_replace(array('вка','ая'),array('вкой','ой'),$model->parking->desc),'UTF-8').','
                     .' без посредников по стоимости аренды от собственника Москвы.'
                    : '  | Аренда коммерческой недвижимости по видам паркинга в Москве без комиссии. Снять нежилое офисное, банковское, торговое'
                     .' помещение либо бизнес центр, банк, здание, особняк целиком с различной видом парковки,'
                    .' без посредников по стоимости аренды от собственника Москвы.');

if ($model->parking) $this->pageDescription= /*trim($model->parking->seo_desc)<>'' 
                           ? 'Здесь отобраны предложения аренды коммерческой недвижимости с парковкой '.mb_strtolower($model->parking->title,'UTF-8').' в Москве. '
                             .$model->parking->seo_desc 
                           :*/ 'Здесь отобраны предложения аренды коммерческой недвижимости с парковкой '.mb_strtolower($model->parking->title,'UTF-8').' в Москве.'
                             ...
                             .' медцентре, банке Москвы, а также аренду бизнес центров, банков, особняков и зданий целиком только вида парковки '.mb_strtolower($model->parking->title,'UTF-8').'.'
                             .' Аренда коммерческой недвижимости обладающей '.mb_strtolower(str_replace(array('вка','ая'),array('вкой','ой'),$model->parking->desc),'UTF-8').', отличается также'.
...
else $this->pageDescription= 'Здесь отобраны предложения аренды коммерческой недвижимости по парковке в Москве. Предложения включают аренду' 
                           ...
                           .' аренду бизнес центров, банков, особняков и зданий целиком отличающихся между собой видом парковки. Кроме этого аренда коммерческой недвижимости'
                           .' отличается видом, размещением, адресом, назначением, свойствами, удаленностью от метро, площадью, стоимостью, валютой.';
        
if ($model->parking->title) $this->pageKeywords=explode(',', 
                                                    'аренда '.mb_strtolower($model->parking->title,'UTF-8').' в москве'
                                                 .', аренда '.mb_strtolower($model->parking->title,'UTF-8')                                                 
                                                 .', аренда '.mb_strtolower($model->parking->title,'UTF-8')                                                 
                                                 //.', аренда офиса в москве '.mb_strtolower($model->parking->abbr,'UTF-8')
                                                 .', аренда в москве парковка '.mb_strtolower($model->parking->abbr,'UTF-8')
                                                 .', '.mb_strtolower($model->parking->title,'UTF-8')
                                                 //.', '.mb_strtolower($model->parking->abbr,'UTF-8')
                                                 .', снять, каталог недвижимости, коммерческая недвижимость, недвижимость, аренда, паркинг, москва, в москве, москвы, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');                   
else $this->pageKeywords=explode(',', 'снять, аренда в москве, каталог недвижимости, коммерческая недвижимоcть, аренда, парковка, все типы паркинга, москва, в москве, москвы, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');    
...
if ($model->parking) {
    $this->breadcrumbs=array(	
         ...
         'по виду паркинга '=>array('realestates/parking'),
	 'Аренда коммерческой недвижимости с '.mb_strtolower(str_replace(array('вка','ая'),array('вкой','ой'),$model->parking->desc),'UTF-8').' в Москве',
    );
} else {
    $this->breadcrumbs=array(	
         Yii::t('menu-adm','List Realestates')=>array('index'),
	 'Аренда коммерческой недвижимости по видам паркинга',
    );
}
...
<?php
    // Переменная $extraRowExpression    
    include __DIR__ . '/include/_var_extrowexp_parking.php';
?>
...
<?php 
    $model->search()->pagination->route = Yii::app()->createUrl('/realestates/parking', array('id'=>$model->parking_id));
?>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	...
	'dataProvider'=>$model->search($short ? 8 : 10),
        ...
        //'ajaxUrl'=>Yii::app()->createUrl('/realestates/parking'),
        'ajaxUrl'=>$property->is_all ? Yii::app()->createUrl('/realestates/parking') : Yii::app()->createUrl('/realestates/parking',array('id'=>$model->parking_id)),
	...
	'columns'=>array(
		...
		'class'=>'CButtonColumn',                
           ...
           array( 'name'=>'parking_id',
                  ...
                  'header'=>Yii::t('all','Парковка'),
                  'type'=> 'html',
                  'filter'=>CHtml::activeHiddenField($model,'parking_id'),                  
                  'value'=>'$data->parking->desc',   
                  'htmlOptions'=>array('style'=>'text-align:center;width:10px;font-size:0.7%'/*30px*/),
                  'headerHtmlOptions'=>array('width'=>'10'/*30px*/),
