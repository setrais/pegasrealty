...
if ($offer) $this->pageTitle=$offer->seo_title 
                             ? $offer->seo_title
                             : Yii::t('all','Каталог коммерческой недвижимости ').': аренда коммерческой недвижимости площадью от '.$offer->init_value.' м2 до '
                               .$offer->final_value.' м2';//.' - '.str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));
else $this->pageTitle=Yii::t('all','Каталог коммерческой недвижимости ')
                    .': аренда коммерческой недвижимости все площадя';//.' - '.str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));str_replace('офисов','офиса', Yii::t('all',Yii::app()->name));

if ($offer) $this->pageDescription=$offer->seo_desc 
                             ? $offer->seo_desc 
                             : Yii::t('all','Здесь отобраны предложения по аренде коммерческой недвижимости площадью от '.$offer->init_value.' м2 до '.$offer->final_value.' м2');                   
else $this->pageDescription=Yii::t('all','Здесь отобраны предложения по аренде коммерческой недвижимости по всем площадям');                   
...
$this->breadcrumbs=array(	
	 ...
	 $offer ? 'площадь от '.$offer->init_value.' м2 до '.$offer->final_value.' м2' : 'все площадя',	 
);
...
<h1>
<?php if ($offer) echo Yii::t('menu-adm','List Realestates')
                    .': аренда коммерческой недвижимости'.' площадью от '.$offer->init_value.' м2 до '.$offer->final_value.' м2';
   else echo Yii::t('menu-adm','List Realestates').': аренда коммерческой недвижимости'.' с разной площадью';
?>
</h1>
<?php if ($offer->anons) {  ?>    
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	'id'=>'realestates-grid',          
	'dataProvider'=>$model->search($short ? 5 : 10),
