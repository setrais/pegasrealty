...
$this->pageTitle= Yii::t('all','Коммерческая недвижимость')
                    .($model->valute
                    ? ' | Аренда коммерческой недвижимости Москвы '.mb_strtolower($model->valute->desc,'UTF-8').' ('.$model->valute->title.').'
                     .' Снять нежилое помещение в '.mb_strtolower($model->valute->title,'UTF-8').' в особняке, бизнес центре, банке, жилом доме, комлексе, парке'
                     .' либо здание в Москве без посредников от собственника.'
                    : '  | Аренда коммерческой недвижимости по валюте в Москве без комиссии. Снять нежилое офисное, банковское, торговое'
                     .' помещение либо бизнес центр, банк, здание, особняк целиком с различной валютой стоимости аренды,'
                    .' без посредников от собственника Москвы.');

if ($model->valute) $this->pageDescription= /*trim($model->tax->seo_desc)<>'' 
                           ? 'Здесь отобраны предложения по аренде коммерческой недвижимости с налогообложением '.$model->tax->abbr.' в Москве. '
                             .$model->tax->seo_desc 
                           :*/ 'Здесь отобраны предложения по аренде коммерческой недвижимости Москвы с стоимостью '
                             .mb_strtolower($model->valute->desc,'UTF-8').' ('.$model->valute->title.').'
                             ...
                             .' медцентре, банке Москвы, а также включают аренду бизнес центров, банков, особняков и зданий целиком в '.mb_strtolower($model->valute->title,'UTF-8').'.'
                             .' Аренда коммерческой недвижимости '.mb_strtolower($model->valute->desc,'UTF-8').', отличается по '.
                              ' виду недвижимости, объекту размещения, классу, назначению, свойствам, удаленности от метро, площади, стоимости.';                   
else $this->pageDescription= 'Здесь отобраны предложения аренды коммерческой недвижимости по валюте расчетов за аренду в Москве. Предложения включают аренду' 
                           ...
                           .' аренду бизнес центров, банков, особняков и зданий целиком отличающихся между собой валютой стоимости и имеющих при этом '
                           .' различный вид, размещение, адресс, назначение, класс, свойства, удаленность от метро, площадь, стоимость.';

if ($model->valute->title) $this->pageKeywords=explode(',', 
                                                        'аренда в валюте '.mb_strtolower($model->valute->abbr,'UTF-8')
                                                     .', аренда москва '.mb_strtolower($model->valute->abbr,'UTF-8')
                                                     .', аренда в москва '.mb_strtolower($model->valute->abbr,'UTF-8')
                                                     .', аренда москва '.mb_strtolower($model->valute->desc,'UTF-8')
                                                     .', аренда в москва '.mb_strtolower($model->valute->desc,'UTF-8')
                                                     .', аренда стоимость '.mb_strtolower($model->valute->desc,'UTF-8')
                                                     .', аренда в москве стоимость '.mb_strtolower($model->valute->desc,'UTF-8')
                                                     .', аренда офиса москва стоимость '.mb_strtolower($model->valute->desc,'UTF-8')
                                                     .', '.mb_strtolower($model->valute->abbr,'UTF-8')
                                                     .', '.mb_strtolower($model->valute->title,'UTF-8')
                                                     .', снять, каталог недвижимости, недвижимость, коммерческая недвижимость, аренда недвижимости, аренда коммерческой недвижимости, аренда коммерческой недвижимости в москве, аренда коммерческой недвижимости москвы, аренда коммерческая недвижимость москва, стоимость, москва, москвы, москве, аренда, валюта, валюте, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');                   
else $this->pageKeywords=explode(',', 'снять, аренда в москве, каталог недвижимости, коммерческая недвижимоcть, аренда недвижимости, аренда коммерческой недвижимости, аренда коммерческой недвижимости в москве, аренда коммерческой недвижимости москвы, аренда коммерческая недвижимость москва, аренда, москва, москвы, в москве, usd, eur, rud, $, руб, €, евро, район, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');    
...
if ($model->valute) {
    $this->breadcrumbs=array(	
         ...
         'по валюте'=>array('realestates/valute'),
	 'Аренда коммерческой недвижимости Москвы, '.mb_strtolower($model->valute->desc,'UTF-8'),        
    );
} else {
    $this->breadcrumbs=array(	
         Yii::t('menu-adm','List Realestates')=>array('index'),
	 'Аренда коммерческой недвижимости по валюте расчетов за аренду',
    );
}
?>
...
<?php $this->renderPartial('_search',
				 ...
				 'model'=>$model, 
                                 //'is_all'=>array('valute'=>$property->is_all)
                                )
                          );
?>
</div><!-- search-form -->
...
<?php
    // Переменная $extraRowExpression    
    include __DIR__ . '/include/_var_extrowexp_valute.php';
?>
...
<?php 
    $model->search()->pagination->route = Yii::app()->createUrl('/realestates/valute', array('id'=>$model->valute_id));
?>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	...
	'dataProvider'=>$model->search($short ? 8 : 10),
        ...
        //'ajaxUrl'=>Yii::app()->createUrl('/realestates/valute'),
        'ajaxUrl'=>$property->is_all ? Yii::app()->createUrl('/realestates/valute') : Yii::app()->createUrl('/realestates/valute',array('id'=>$model->valute_id)),
		...
		'class'=>'CButtonColumn',                
          ...
          array(  'name'=>'price',
           ...
           array( 'name'=>'valute_id',
                  ...
                  'type'=> 'html',
                  'filter'=>CHtml::activeHiddenField($model,'valute_id'),                  
                  'value'=>'$data->valute->title',  
                  'htmlOptions'=>array('style'=>'ttext-align:right;width:50px;'/*30px*/),
