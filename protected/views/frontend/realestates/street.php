...
$this->pageTitle= Yii::t('all','Коммерческая недвижимость')
                    .($model->street 
                    ? ' | Аренда коммерческой недвижимости '.$model->street->getFullName('по').' в Москве без комиссии.'
                     .' Снять нежилое помещение либо здание целиком на '.$model->street->title.','
                     .' без посредников по стоимости аренды от собственника Москвы.'
                    : '  | Аренда коммерческой недвижимости по расположению в Москве без комиссии. Снять нежилое недвижимость'
                     .' на улице, аллее, бульваре,  городке, шоссе, квартале, набережной, переулке, площаде, проспекте, проезде,'
                     .' просеке, тупике  Москвы, без посредников от собственника.');

if ($model->street) $this->pageDescription= trim($model->street->seo_desc)<>'' 
                                        ? 'Здесь отобраны предложения аренды коммерческой недвижимости '.$model->street->getFullName('на').' в Москве. '
                                          .$model->street->seo_desc 
                                        : 'Здесь отобраны предложения по аренде коммерческой недвижимости '.$model->street->getFullName('на').' в Москве.'
                                         ...
                                         .' медцентре, банке, а также аренду бизнес центров, банков, особняков и зданий целиком по '.$model->street->title.' Москвы.'
                                         .' Аренда коммерческой недвижимости '.$model->street->getFullName('на').' также отличается классом, видом, объектом размещения,'
                                         .' назначением, свойствами, удаленностью от метро, площадью, стоимостью, валютой.';                   
else $this->pageDescription=Yii::t('all','Здесь отобраны предложения аренды коммерческой недвижимости по расположению в Москве. Предложения включают аренду нежилой недвижимости на' 
                                        .' улице, аллее, бульваре,  городке, шоссе, квартале, набережной, переулке, площаде, проспекте, проезде, просеке, тупике Москвы.'
                                        .' Аренда коммерческой недвижимости также отличается'
                                        .' между собой видом, классом, объектом размещения, назначением, свойствами, удаленностью от метро, площадью, стоимостью, валютой.');

if ($model->street->name) $this->pageKeywords=($model->street->seo_keywords 
                                               ? array_merge(
                                                explode(',',' аренда в москве на '.mb_strtolower($model->street->title,'UTF-8')
                                                         .', аренда в москве на '.mb_strtolower($model->street->name,'UTF-8')
                                                         .', аренда в москве на '.mb_strtolower($model->street->name,'UTF-8').' '.mb_strtolower($model->street->getFullName(null,false),'UTF-8')
                                                         .', аренда в москве '.mb_strtolower($model->street->getFullName('на'),'UTF-8')                                                        
                                                        
                                                         .', аренда москва на '.mb_strtolower($model->street->name,'UTF-8')
                                                         .', аренда офиса москва на '.mb_strtolower($model->street->title,'UTF-8')
                                                                                                                                                                        
                                                         .', аренда '.mb_strtolower($model->street->name,'UTF-8').' '.mb_strtolower($model->street->getFullName(null,false),'UTF-8')
                                                         .', аренда '.mb_strtolower($model->street->getFullName(null,false),'UTF-8').' '.mb_strtolower($model->street->name,'UTF-8')
                                                        
                                                         .', аренда '.mb_strtolower($model->street->name,'UTF-8')                                                         
                                                         .', аренда '.mb_strtolower($model->street->title,'UTF-8')    

                                                         .', '.mb_strtolower($model->street->title,'UTF-8')                 
                                                         .', '.mb_strtolower($model->street->name,'UTF-8')
                                                         .', снять, каталог недвижимости, коммерческая недвижимость, недвижимость, аренда недвижимости, аренда коммерческой недвижимости, аренда коммерческой недвижимости в москве, аренда коммерческой недвижимости москвы, аренда коммерческая недвижимость москва, аренда, в москве, москва, москвы, район, улица, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц'),
...
else $this->pageKeywords=explode(',', 'снять, аренда в москве, каталог недвижимости, недвижимость, аренда недвижимости, аренда коммерческой недвижимости, аренда коммерческой недвижимости в москве, аренда коммерческой недвижимости москвы, аренда коммерческая недвижимость москва, аренда, в москве, москва, москвы, улица, ул., аллея, бульвар, городок, шоссе, квартал, набережная, переулок, пер., пл., площадь, пр., проспект, проезд, просек, тупик, район, улица, метро, от метро, планировка, парковка, вид, класс, налоговая, налогообложение, отдельный вход, кк, свойства, арендная ставка, стоимость в месяц');                   
...
if ($model->street) {
    $this->breadcrumbs=array(	
	 ...
	 "Аренда коммерческой недвижимости ".$model->street->getFullName('на')." в Москве",
    );
} else {
    $this->breadcrumbs=array(	
         Yii::t('menu-adm','List Realestates')=>array('index'),
	 'Аренда коммерческой недвижимости по расположению',
    );
}
...
<?php echo //Yii::t('all','Коммерческая недвижимость').' | '.
           ($model->street 
                    ? 'Аренда коммерческой недвижимости '.$model->street->getFullName('по').' в Москве без комиссии.'
                     .' Снять нежилое помещение или здание целиком '.$model->street->getFullName('на')                       
                     .' без посредников по стоимости аренды от собственника Москвы.'
                    : 'Аренда коммерческой недвижимости по расположению в Москве без комиссии.'
                     .' Снять нежилое помещение либо здание целиком на улице, аллее, бульваре,  городке, шоссе, квартале,'
                     .' набережной, переулке, площаде, проспекте, проезде, просеке, тупике  Москвы, без посредников'
...
<?php
    // Переменная $extraRowExpression    
    include __DIR__ . '/include/_var_extrowexp_street.php';
?>
...
<?php 
    $model->search()->pagination->route = Yii::app()->createUrl('/realestates/street', array('id'=>$model->street_id));
?>
...
<?php $this->widget('ext.groupgridview.GroupGridView'/*zii.widgets.grid.CGridView'*/, array(
	...
	'dataProvider'=>$model->search($short ? 8 : 10),
        ...
        //'ajaxUrl'=>Yii::app()->createUrl('/realestates/street'),
        'ajaxUrl'=>$property->is_all ? Yii::app()->createUrl('/realestates/street') : Yii::app()->createUrl('/realestates/street',array('id'=>$model->street_id)),
	...
	'columns'=>array(
		...
		'class'=>'CButtonColumn',                
           ...
           array( 'name'=>'street_id',
                  'header'=>Yii::t('all','Улица'),  
                  'type'=> 'html',
                  'filter'=>CHtml::activeHiddenField($model,'street_id'),                  
                  'value'=>'$data->street->title;', 
                  'htmlOptions'=>array('style'=>'width:100px;'/*60px*/),
                  'headerHtmlOptions'=>array('width'=>'100'/*60px*/),
           ...
           array( 'name'=>'street_id',
                  ...
                  'value'=>'$data->street->title;',
                  'htmlOptions'=>array('style'=>'width:100px;'/*60px*/),
                  'filter'=>CHtml::listData(Streets::model()->findAll(
                                            array('select'=>'t.id,t.title',/*"condition"=>"(ACT IS NULL OR ACT=1) AND (DEL IS NULL OR DEL=0)",*/
