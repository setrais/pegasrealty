      ...
      <td class="Image_map">
        ...
        <td class="text_map" style="padding-right: 10px;padding-top:0px; text-align: right">  
          ...
          <?php foreach($mets as $met) { ?>    
                  //if (!metroMap.isStantionSelected(<?=$met->mapid;?>)) { 
                    //metroMap.stantionSwitch(<?=$met->mapid;?>,true,true);
                  //}
                  metroMap.stantionSelect(<?=$met->mapid;?>,true,true);
                  /*$('area#link_<?=$met->mapid;?>').trigger('click');*/
          <?php   $arr[]=$met->mapid;   ?>         
                  <?/*<li id="selmetro_<?=$met->mapid;?>">
                                         <input type="hidden" name="metro[]" value="<?=$met->mapid;?>" >
                                         <a href="javascript: RemoveMetro(<?=$met->mapid;?>)" ><?=$met->title;?></a> 
                                         </li>*/?>
                                             
         <?php  } ?>
            $(document).ready(function() { 
             var station = <?php echo json_encode($arr);?>; 
             $('#stantions_text span').each(function() {                
                 var mapid = $(this).attr('id').replace('stantionText_','');                 
                 if($.inArray(mapid,station)<0) {
                     metroMap.stantionUnSelect(mapid);                     
                 }                 
             });
             metroMap.save();
             });             
