<? if ($model) {
        ...
        $records = Properties::model()->active()
                                    ->mapfields()                                                             
                                    ->firstcreate()
        ...
        $model = Properties::model()->mapfields()                                                             
                                    ->realex()
                                    ->firstcreate()
                                    ->findAll();
   ...
   if ($records) {
       ...
       foreach ($records as $record) {
              if (!isset($extvid)) {
                  $li_record[]= Chtml::tag('li', array(), Chtml::link( 'Аренда коммерческой недвижимости с свойством '.$record->title.' в Москве'/*$clas->realestateClass->title*/, 
                                                                      Yii::app()->createUrl( 'realestates/property', array( 'id' => $record->id )), 
                                                                      array( 'title' => 'Аренда коммерческой недвижимости c свойством '.$record->title.' в Москве')));
              }else{
                  $li_record[]= Chtml::tag('li', array(),Chtml::link( 'Аренда '.$extvid->namewhats.' Москвы с свойством '.$record->title, 
                                                                     Yii::app()->createUrl( 'realestates/property', 
                                                                             array( 'id' => $record->id, 
                                                                              'property' => $extvid->type, 
                                                                                   'pid' => $extvid->id)),                        
                                                                     array('title'=>'Аренда '.$extvid->namewhats.' с свойством '.$record->title.', в Москве')));
              }                           
       }              
...
?>
    ...
    <h2 class="list <?/*fs-11 m-0 p-0 c-black inline-valn ff-tahoma m-b5 bold*/?>">   
        ...
        Снять в аренду <?=isset($extvid) ? $extvid->namewhat 
                                         : 'коммерческую недвижимость'/*офис, нежилое помещение или здание'*/;?> в Москве по свойствам:
        <?php } ?>                                 
    </h2>     
